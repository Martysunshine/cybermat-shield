...
| **File writes** | Never modifies your source tree — only writes to the `.cybermat/` output directory |
...
- **Harmless marker injection** — the reflection analyzer injects a unique string like `cybermat-probe-abc123` in query parameters. This string cannot cause harm in any common output context and is not a real XSS payload.
...
4. Keeping `.cybermat/auth/` storageState files out of version control (the tool appends this to `.gitignore` automatically via `cybermat init`)
