...
describe('buildFileInventory — file inclusion', () => {
  test('includes Python .py file with correct language/fileKind', async () => {
    write('app/main.py', 'print("hello")\n');
    const { files } = await buildFileInventory(tmpDir, CONFIG);
  ...
  test('includes Go .go file', async () => {
  ...
  test('includes Dockerfile (no extension) with language=dockerfile', async () => {
  ...
  test('includes Makefile (no extension) with fileKind=script', async () => {
    write('Makefile', 'all:\n\techo done\n');
    const { files } = await buildFileInventory(tmpDir, CONFIG);
  ...
  test('includes Jenkinsfile (no extension) with fileKind=ci_cd', async () => {
  ...
  test('includes .github/workflows/ci.yml with fileKind=ci_cd and ecosystem=github-actions', async () => {
  ...
  test('includes Terraform .tf file with language=terraform', async () => {
  ...
  test('includes .env.local with language=env and fileKind=env', async () => {
  ...
  test('includes package.json with fileKind=dependency_manifest', async () => {
  ...
  test('includes firestore.rules with fileKind=security_rules', async () => {
  ...
  test('includes go.mod with fileKind=dependency_manifest', async () => {
  ...
  test('includes pom.xml with fileKind=dependency_manifest', async () => {
  ...
  test('includes Cargo.toml with ecosystem=rust', async () => {
  ...
  test('includes Gemfile with ecosystem=ruby', async () => {
  ...
  test('includes nginx.conf with language=config', async () => {
  ...
  test('includes requirements.txt', async () => {
...
describe('buildFileInventory — binary and large file exclusion', () => {
  test('skips file containing null bytes', async () => {
    ...
    const { files, skippedByReason } = await buildFileInventory(tmpDir, CONFIG);
  ...
  test('skips file over maxFileSizeBytes', async () => {
    ...
    const { files, skippedByReason } = await buildFileInventory(tmpDir, smallConfig);
  ...
  test('skips files matching ignoreFiles config', async () => {
    write('src/secret-backup.ts', 'const x = 1;\n');
    const cfg: ScannerConfig = { ...CONFIG, ignoreFiles: ['secret-backup'] };
    const { files, skippedByReason } = await buildFileInventory(tmpDir, cfg);
  ...
  test('FileInventoryResult includes skipped count and skippedByReason', async () => {
    const result = await buildFileInventory(tmpDir, CONFIG);
